...
switch ($action) {
    case 'create':
        ...
        if($phone_check){
            ...
            createNewInspector($user_name, $phone_first, $mail, $type);
        }
        break;
    case 'update':
        if(!$phone_first){
        ...
        updateInspectorById($user_name, $idToEdit, $phone_first, $mail, $active);
        break;
    default;
}
if (isset($_GET['edit'])) {
    ...
    $inspector = getInspectorById($idToEdit);
    if(!$inspector){
        header('Location: /');
        exit;
    }
    $project = getProjectByProjectId($inspector['i_project_id']);

    $title = "עריכת מבקר - " . $inspector['st_user_name'];
    $action = "update";
    $btn = "שמור";
} else {
    $title = "הוספת מבקר חדש";
    $action = "create";
    $btn = "הוסף מבקר";
}
...
<div class="main_container editor">
    <div class="main_container__header">
        ...
        <a class="btn outline_btn" href="inspectors.php">חזרה</a>
    </div>
    <div class="main_container__box">
        <form class="form_editor" action="" method="post">
            ...
            <input type="hidden" name="project_id" value="<?= isset($project_id) ? $project_id : $inspector['i_project_id'] ?>">

            <label for="user_name">*שם מבקר</label>
            <input type="text" name="user_name" id="user_name" placeholder="הזן את שם מבקר"
                   value="<?= isset($idToEdit) ? html_entity_decode($inspector['st_user_name']) : '' ?><?= isset($user_name) && !isset($idToEdit) ? $user_name : '' ?>" required
            ...
            <input class="input_ltr" type="tel" name="phone_first" id="phone_first" placeholder="הזן את מספר הנייד"
                   value="<?= isset($idToEdit) ? $inspector['st_phone_first'] : '' ?><?= isset($phone_first) && !isset($idToEdit) ? $phone_first : '' ?>" pattern="^[0-9\-\+]{9,15}$" required
            ...
            <input class="input_ltr" type="email" name="mail" id="mail" placeholder="הזן <?= $mail_title ?>"
                   value="<?= isset($idToEdit) ? $inspector['st_mail'] : '' ?><?= isset($mail) && !isset($idToEdit) ? $mail : '' ?>" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}$"
            ...
            <?php if(isset($idToEdit)) : ?>
                <label for="active" class="container" style="width: 100%">משתמש פעיל
                    <input type="checkbox" id="active" name="active" <?= $inspector['i_active'] == 1 ? 'checked' : '' ?>>
