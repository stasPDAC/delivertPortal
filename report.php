...
<div id="welcome" class="modal" <?= $user_type == 4 && $client['i_status'] == 1 && !isset($_COOKIE['first_time']) ? 'style="display:flex"' : '' ?>>
    <div class="modal-content">
        <div id="view_1" class="views" <?= $user_type == 4 && $client['i_status'] == 1 && !isset($_COOKIE['first_time']) ? 'style="display:block"' : '' ?>>
        ...
        <div class="flex">
            <?php if($user_type == 4) : ?>
                ...
                <?php if($client['i_status'] == 1) : ?>
                    ...
                    <?php else : ?>
                        <div><a href="examiner.php?id=<?=$report_serial?>" class="btn_header" title="הוספת פרטי מאנדס"><?=$icons['add_engineering']?></a></div>
                    <?php endif ?>
                    <form id="pdf_upload_form" method="post" enctype="multipart/form-data">
                        <div class="file-upload">
                            <div class="file-select btn outline_btn">
                                ...
                                <input accept="application/pdf" onchange="$('#pdf_upload_form').submit()" type="file" name="file" size="50">
                            </div>
                        </div>
                    </form>
                    ...
                    <form action="" method="post">
                        ...
                        <input type="hidden" name="report_serial" value="<?= $report_serial ?>">
                        <button class="btn" type="submit" onclick="if (!confirm('לאחר סיום לא יהיה ניתן לערוך את הדוח')) return false;">סיום דוח</button>
                    </form>
                <?php endif; ?>
            ...
            <?php endif ?>
            <?php if($client['i_status'] != 1) : ?>
                <a href="pdf_clientReport.php?id=<?=$report_serial?>" class="btn_header" target="_blank" title='דו"ח ביקורת ליקויים'><?=$icons['client_report_pdf']?></a>
            <?php endif ?>
            <?php if($user_type == 4 && $client['i_status'] == 3) : ?>
                <a href="pdf_exportReport.php?id=<?=$report_serial?>" class="btn_header" target="_blank" title='דו"ח ביקורת איכות'><?=$icons['report_pdf']?></a>
