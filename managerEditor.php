...
switch ($action) {
    case 'create':
        ...
        if($phone_check){
            ...
            createNewManager($user_name, $phone_first, $mail, $type);
        }
        break;
    case 'update':
        if(!$phone_first){
        ...
        updateManagerById($user_name, $idToEdit, $phone_first, $mail, $active);
        break;
    default;
}
if (isset($_GET['edit'])) {
    ...
    $title = "עריכת מנהל פרויקט - " . $client['st_user_name'];
    $action = "update";
    $btn = "שמור";
} else {
    $title = "הוספת מנהל פרויקט חדש";
    $action = "create";
    $btn = "הוסף מנהל פרויקט";
}
...
<div class="main_container editor">
    <div class="main_container__header">
        ...
        <a class="btn outline_btn" href="managers.php">חזרה</a>
    </div>
    <div class="main_container__box">
        <form class="form_editor" action="" method="post">
            ...
            <label for="user_name">*שם המנהל פרויקט</label>
            <input type="text" name="user_name" id="user_name" placeholder="הזן את שם המנהל"
                   value="<?= isset($idToEdit) ? html_entity_decode($client['st_user_name']) : '' ?><?= isset($user_name) && !isset($idToEdit) ? $user_name : '' ?>" required
